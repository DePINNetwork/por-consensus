...
import (
	...
	metricsgen "github.com/depinnetwork/por-consensus/scripts/metricsgen" //nolint:revive // this only works with metricsgen in front
)
...
func TestParseAliasedMetric(t *testing.T) {
	aliasedData := `
			...
			import(
				mymetrics "github.com/depinnetwork/por-consensus/libs/metrics"
			)
			type Metrics struct {
				m mymetrics.Gauge
			}
